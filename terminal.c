...
/* Write a literal character to the display -- no controls. */
static void term_out_litchar(Terminal *term, unsigned long c)
{
...
#define CLAMP(arg, lim) ((arg) = ((arg) > (lim)) ? (lim) : (arg))
			...
			compatibility(ANSI);
			{
			    ...
			    if (term->vttest_decmodes) term->lastc = 0x3020;
			}
			seen_disp_event(term);
			break;
