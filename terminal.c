...
static void power_on(Terminal *term, int clear)
{
    ...
    term->utf = (term->ucsdata->line_codepage==CP_UTF8);
...
void term_reconfig(Terminal *term, Conf *conf)
{
    ...
    /* NOTE: The structure ucsdata isn't really ready yet ... */
    term->utf = (term->ucsdata->line_codepage==CP_UTF8);
    if (!term->utf && conf_get_int(term->conf, CONF_utf8_override))
	term->utf = term->ucsdata->utf8_locale;
...
#define CLAMP(arg, lim) ((arg) = ((arg) > (lim)) ? (lim) : (arg))
		      ...
		      case 'm':       /* SGR: set graphics rendition */
			{
				...
				switch (def(term->esc_args[i], 0)) {
				  ...
				  case 27:	/* disable reverse video */
				    ...
				    break;
				  case 51:
				    /* Revert the effects of CSI=F and CSI=G */
				    compatibility(SCOANSI);
		...
		switch (c) {
		  ...
		  case 'D':
		     ...
		     *
		     * From VT102 manual (and the VT52 chargen ROM):
		     *       137 _  Blank             - Same
		     *       140 `  Reserved          - dͨ or ㏅ - TL=c BR=d
